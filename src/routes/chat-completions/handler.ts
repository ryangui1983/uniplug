...
import { createKiroChatCompletions } from "~/services/kiro/chat-completions"
...
export async function handleCompletion(c: Context) {
  ...
  consola.info(
    `[Request1] provider: ${state.provider}, model: ${payload.model}`,
  )
  ...
  const response =
    state.provider === "kiro" ?
      await createKiroChatCompletions(payload)
